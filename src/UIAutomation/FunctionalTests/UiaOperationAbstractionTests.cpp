...
namespace UiaOperationAbstractionTests
{
    ...
    public:
        ...
        inline winrt::com_ptr<IUIAutomationElement> getElement(DWORD pid)
        {
            winrt::com_ptr<IUIAutomation> automation;
            THROW_IF_FAILED(::CoCreateInstance(__uuidof(CUIAutomation8), nullptr, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(automation.put())));
            ...
            HRESULT hr = automation->ElementFromHandle(handle, element.put());

            wil::com_ptr<IUIAutomationTreeWalker> spWalker;
            hr = automation->get_ControlViewWalker(&spWalker);
            winrt::com_ptr<IUIAutomationElement> uiaChild;
            winrt::com_ptr<IUIAutomationElement> uiaChild1;
            winrt::com_ptr<IUIAutomationElement> uiaChild2;
            spWalker->GetFirstChildElement(element.get(), uiaChild.put());
            spWalker->GetNextSiblingElement(uiaChild.get(), uiaChild1.put());
            spWalker->GetNextSiblingElement(uiaChild1.get(), uiaChild2.put());
            RECT rec;
            hr = uiaChild2->get_CurrentBoundingRectangle(&rec);

            THROW_IF_FAILED(hr);
            return uiaChild2;
        }

        UiaUint dfs1(UiaElement node)
        {
            auto operationScope = UiaOperationScope::StartOrContinue();
            operationScope.BindInput(node);
            //UiaElement child1 = node.GetFirstChildElement();
            UiaArray<UiaElement> arr;
            UiaArray<UiaElement> stackArr;
            auto name = node.GetName();
            //LogOutput(name.get());
            
            UiaBool continueLoop{ true };
            UiaUint i = 0;
            UiaUint j = 0;
            stackArr.Append(node);
            UiaElement child = node.GetFirstChildElement();
                    //stackArr.Append(child);
            UiaString elementText{ L"its there" };
            operationScope.While(
                [&]()//condition
                {
                ...
                [&]()//body
                {
                    UiaElement sib = stackArr.GetAt(i);
                    stackArr.RemoveAt(i);
                    //i += 1;
                    operationScope.While(
                        [&]()//condition
                        ...
                        [&]()//body
                        {
                            auto boundingRect = sib.GetBoundingRectangle(false);
                            auto x = boundingRect.GetX();
                            auto y = boundingRect.GetY();
                            auto w = boundingRect.GetWidth();
                            auto h = boundingRect.GetHeight();
                            /*UiaDouble xc = 850;
                            UiaDouble yc = 940;
                            UiaDouble wc = 1620;
                            UiaDouble hc = 1080;*/
                            ...
                            UiaDouble hcf = y ;
                            hcf += h;
                            UiaBool temp = ((x >= xc) && (y >= yc) && (wcf <= wc) && (hcf <= hc));
                            operationScope.If(temp, [&]()
                                {
                                    UiaElement child = sib.GetFirstChildElement();

                                    operationScope.If(!child.IsNull(), [&]()
                                        {
                                            stackArr.Append(child);
                                        }, [&]()
                                        {
                                            auto tPattern = sib.GetTextChildPattern();
                                            auto textRange = tPattern.GetTextRange();
                                            
                                            elementText = textRange.GetText(-1);
                                        });
                                });
                            operationScope.If(!temp, [&]()
                                {
                                    ...
                                    operationScope.If(!child.IsNull() && j<57, [&]()
                                        {
                                            stackArr.Append(child);
                                            j += 1;
                                        });
                            ...
                            //&& !((x <= xc && wcf <= wc) || (y <= yc && hcf <= hc) || (x >= xc && wcf >= wc) || (y >= yc && hcf >= hc))
                            /*operationScope.If(!temp , [&]()
                                {
                                    UiaElement child = sib.GetFirstChildElement();
                            ...
                            sib = sib.GetNextSiblingElement();      
                        });
                });
            operationScope.BindResult(j);
           
            operationScope.Resolve();
            return j;
        }

        UiaUint dfs(UiaElement node)
        {
            auto operationScope = UiaOperationScope::StartOrContinue();
            operationScope.BindInput(node);
            UiaArray<UiaElement> arr;
            ...
            operationScope.While(
                [&]()//condition
                {
                    return stackArr.Size() > 0 && i<500;
                },
                [&]()//body
                {
                    UiaElement sib = stackArr.RemoveAt(0);
                    i += 1;
                    operationScope.While(
                        [&]()//condition
                        {
        ...
        void sampleTest()
        {
            ...
            if(element !=NULL)
            {
                ...
                auto text = dfs(element.get());
                LogOutput(text);
            }
        }
        void sampleTest2(DWORD pid)
        {
            winrt::com_ptr<IUIAutomation> automation;
            THROW_IF_FAILED(::CoCreateInstance(__uuidof(CUIAutomation8), nullptr, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(automation.put())));
            ...
            spWalker->GetNextSiblingElement(uiaChild1.get(), uiaChild2.put());
            BSTR name;
            hr = uiaChild2->get_CurrentName(&name);
            

            std::vector< winrt::com_ptr<IUIAutomationElement>> stackArr;
            winrt::com_ptr<IUIAutomationElement> node = uiaChild2;
            stackArr.push_back(node);
            wil::shared_bstr elementText;
            unsigned int i = 0;
            BSTR MyBstr = SysAllocString(L"Click here");
            while (i < stackArr.size())
            {
                winrt::com_ptr<IUIAutomationElement> sib = stackArr[i];
                i += 1;
                while (sib)
                {
                    BSTR name;
                    hr = sib->get_CurrentName(&name);
                    LogOutput(name);
                    /*if (wcscmp(name, MyBstr) == 0)
                    {
                        LogOutput(name);
                        RECT boundingRect;
                        sib->get_CurrentBoundingRectangle(&boundingRect);

                    }*/
                    
                    RECT boundingRect;
                    sib->get_CurrentBoundingRectangle(&boundingRect);
                    long x = boundingRect.left, y = boundingRect.top, wcf = boundingRect.right, hcf = boundingRect.bottom;
                    bool temp = ((x >= 700) && (y >= 1300) && (wcf <= 1900) && (hcf <= 1500));
                    /*if (x == (long)729)
                    {
                        BSTR name;
                        sib->get_CurrentName(&name);

                    }*/
                    if (temp)
                    {
                        winrt::com_ptr<IUIAutomationElement> child;
                        spWalker->GetFirstChildElement(sib.get(), child.put());
                        if (!child)
                        {
                            stackArr.push_back(child);
                        }
                        else
                        {
                            winrt::com_ptr<IUIAutomationElement> parent;
                            spWalker->GetParentElement(sib.get(), parent.put());
                            winrt::com_ptr<IUIAutomationTextRange> elementRange;
                            winrt::com_ptr<IUIAutomationTextPattern> pattern;
                            element->GetCurrentPatternAs(UIA_TextPatternId, IID_PPV_ARGS(pattern.put()));
                            pattern->RangeFromChild(child.get(), elementRange.put());
                            elementRange->GetText(-1, &elementText);
                        }
                    }
                    else
                    {
                        ...
                        {
                            winrt::com_ptr<IUIAutomationTextRange> elementRange;
                            winrt::com_ptr<IUIAutomationTextChildPattern> pattern;
                            sib->GetCurrentPatternAs(UIA_TextChildPatternId, IID_PPV_ARGS(pattern.put()));
                            pattern->get_TextRange(elementRange.put());
                            elementRange->GetText(-1, &elementText);
                        }*/
                    
                    }
                    winrt::com_ptr<IUIAutomationElement> sib1;
                    spWalker->GetNextSiblingElement(sib.get(), sib1.put());
                    sib = sib1;
                }
            }
        }

        TEST_METHOD(sampleTestRemote)
        {
            sampleTest();
            //sampleTest2(13772);
        }
